...
int main(int argc, char** argv)
{
  ...
  ros::Publisher pubVehicleOdom = nh.advertise<nav_msgs::Odometry>("/state_estimation", 5); //state_estimation
  ...
  while (status)
  {
    ...
    // */
    // Set the linear velocity (forward motion in the x direction)
    twist_msg.linear.x = vehicleSpeed;  // 1 meter per second
    twist_msg.linear.y = 0.0;
    twist_msg.linear.z = 0.0;

    // Set the angular velocity (rotation around the z-axis)
    twist_msg.angular.x = 0.0;
    twist_msg.angular.y = 0.0;
    twist_msg.angular.z = vehicleYawRate;  // 0.5 radians per second (counterclockwise)

    // Publish the message to the /cmd_vel topic
    huskymotion.publish(twist_msg);
